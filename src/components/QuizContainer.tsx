...
type ReinoNome = "Cemitério" | "Natural" | "Encruzilhada" | "Lira";

interface ResultadoQuiz {
  reino: ReinoNome;
  pontuacoes: Record<ReinoNome, number>;
  reinosEmpatados: ReinoNome[];
}
...
const reinos: ReinoNome[] = ["Cemitério", "Natural", "Encruzilhada", "Lira"];

const resultadoPorOpcao: ReinoNome[][] = [
...
const resultadoDescricao: Record<ReinoNome, string> = {
...
const imagensReinos: Record<ReinoNome, string> = {
  Lira: "/guardiao-padilha.webp",
  Encruzilhada: "/guardiao-sete.webp",
  Natural: "/guardiao-esdras.webp",
  Cemitério: "/guardiao-caveira.webp",
};

const frasesCompartilhamento: Record<ReinoNome, string> = {
  Cemitério:
...
export default function QuizContainer() {
  ...
  const [avisoId, setAvisoId] = useState<number>(0);
  ...
  const handleChange = (index: number, valor: number) => {
    setRespostas((prev) => ({ ...prev, [index]: valor }));
    setFaltantes((prev) => prev.filter((item) => item !== index));
  };

  const calcularResultado = (event: FormEvent<HTMLFormElement>) => {
    ...
    const pontuacoes: Record<ReinoNome, number> = {
      Cemitério: 0,
      Natural: 0,
      Encruzilhada: 0,
      Lira: 0,
    ...
    Object.entries(respostas).forEach(([index, opcao]) => {
      const reinoVotado = resultadoPorOpcao[Number(index)][opcao];
      pontuacoes[reinoVotado] += 1;
    });
  ...
  const getUrlQuiz = () => {
    if (typeof window === "undefined")
      return "https://livroocastelodasmarias.netlify.app/quiz";
